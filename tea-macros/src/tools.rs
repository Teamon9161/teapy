...
use syn::{
    parse_macro_input, parse_quote, FnArg, Generics, Ident, ImplItem, Item, ReturnType, Type,
};

pub(crate) fn pre_impl_parser(
    input: Item,
) -> (
    Ident,
    Box<Type>,
    ...
    if let Item::Impl(item_impl) = input {
        ...
        let (trait_methods, impl_methods): (Vec<_>, Vec<_>) = item_impl
            .items
            .iter()
            .filter_map(|item| {
    ...
    match method {
        MethodType::Map | MethodType::Map2 => {
            ...
            fn_sig.inputs = fn_sig
                ...
                .into_iter()
                .enumerate()
                .filter(|(i, _)| if *i == remove_idx { false } else { true })
                .map(|(_, arg)| arg)
                .collect();
        }
        _ => {}
