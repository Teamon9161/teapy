...
pub type FuncNode<'a> = Arc<dyn Fn(FuncOut<'a>) -> TResult<FuncOut<'a>> + Send + Sync + 'a>;
...
impl<'a> ExprInner<'a> {
    ...
    #[inline]
    pub fn into_out(self, mut ctx: Option<Context<'a>>) -> TResult<Data<'a>> {
    ...
    pub fn view_arr<'b>(&'b self, ctx: Option<&'b Context<'a>>) -> TResult<&'b ArrOk<'a>> {
        if (self.step() > 0) & ctx.is_none() {
            tbail!("Can not view array before evaluate the expression");
        }
    ...
    pub fn view_ols_res<'b>(&'b self, ctx: Option<&'b Context<'a>>) -> TResult<Arc<OlsResult<'a>>> {
        if (self.step() > 0) & ctx.is_none() {
            tbail!("Do not view array before evaluate the expression");
        }
        if ctx.is_some() {
            if self.step() == 0 {
