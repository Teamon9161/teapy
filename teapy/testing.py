...
STABLE_INT_MIN, STABLE_INT_MAX = -1e5, 1e5
dtype_list = [np.float64, np.float32, np.int32, np.int64]
dtype_element_map_stable = {
    np.float64: st.floats(width=16, min_value=STABLE_FLOAT_MIN, max_value=STABLE_FLOAT_MAX),
    np.float32: st.floats(width=16, min_value=STABLE_FLOAT_MIN, max_value=STABLE_FLOAT_MAX),
...
def make_arr(shape=100, nan_p=0.05, unique=False, dtype=None, stable=False):
    ...
    @st.composite
    def draw_arr(draw):
        arr_dtype = draw(st.sampled_from(dtype_list)
                         ) if dtype is None else dtype
        arr = draw(arrays(arr_dtype, shape,
                   elements=dtype_element_map_stable[arr_dtype], unique=unique))
