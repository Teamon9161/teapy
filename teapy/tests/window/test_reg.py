...
def test_ts_reg(arr, window):
    ...
    res2 = pd.Series(arr).rolling(window, min_periods=min_periods).apply(ts_reg)
...
def test_ts_reg_intercept(arr, window):
    ...
    res2 = (
        pd.Series(arr).rolling(window, min_periods=min_periods).apply(ts_reg_intercept)
    )
    assert_series_equal(pd.Series(res1), res2)
...
def test_ts_reg_slope(arr, window):
    ...
    res2 = pd.Series(arr).rolling(window, min_periods=min_periods).apply(ts_reg_slope)
    assert_series_equal(pd.Series(res1), res2)
...
def test_ts_tsf(arr, window):
    ...
    res2 = pd.Series(arr).rolling(window, min_periods=min_periods).apply(ts_tsf)
    assert_series_equal(pd.Series(res1), res2)
