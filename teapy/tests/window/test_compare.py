...
def test_ts_max(arr, window):
    ...
    res2 = pd.Series(arr).rolling(window, min_periods=min_periods).max()
...
def test_ts_min(arr, window):
    ...
    res2 = pd.Series(arr).rolling(window, min_periods=min_periods).min()
    assert_series_equal(pd.Series(res1), res2)
...
def test_ts_argmax(arr, window):
    ...
    res2 = (
        ...
        .apply(lambda x: x.idxmax() - x.index[0] + 1)
    )
    assert_series_equal(pd.Series(res1), res2)
...
def test_ts_argmin(arr, window):
    ...
    res2 = (
        pd.Series(arr)
        .rolling(window, min_periods=min_periods)
        .apply(lambda x: x.idxmin() - x.index[0] + 1)
    )
    assert_series_equal(pd.Series(res1), res2)
