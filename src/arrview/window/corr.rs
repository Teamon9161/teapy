...
impl_arrview!([ArrView1, ArrViewMut1], Number, {
        ...
        usize: Number,
    {
        ...
        let mut n = 0;
        if !stable {
            self.apply_valid_window_with_to(other, out, window, |va, vb, va_rm, vb_rm| {
            ...
            self.stable_apply_valid_window_with_to(other, out, window, |va, vb, va_rm, vb_rm| {
                if va.notnan() && vb.notnan() {
                    n += 1;
                    sum_a.kh_sum(va, c1);
                    sum_b.kh_sum(vb, c2);
