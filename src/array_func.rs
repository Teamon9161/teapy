...
pub fn argsort<T: Number>(arr: ArrayView1<T>, mut out: ArrayViewMut1<usize>) {
    let mut i = 0;
...
pub fn rank<T: Number>(arr: ArrayView1<T>, mut out: ArrayViewMut1<f64>) {
    let len = arr.len();
    if len == 0 {
        return;
    } else if len == 1 {
...
#[allow(unused_assignments)]
pub fn rank_pct<T: Number>(arr: ArrayView1<T>, mut out: ArrayViewMut1<f64>) {
    let len = arr.len();
    if len == 0 {
        return;
    } else if len == 1 {
...
pub fn stable_kurt<T: Number>(arr: ArrayView1<T>, out: &mut f64)
where
    usize: Number,
{
    let mut n = 0usize;
