...
pub fn ts_stable_1d<T: Number> (arr: ArrayView1<T>, mut out: ArrayViewMut1<f64>, window: usize, min_periods: usize, _out_step: usize)
...
pub fn ts_meanstdnorm_1d<T: Number> (arr: ArrayView1<T>, mut out: ArrayViewMut1<f64>, window: usize, min_periods: usize, _out_step: usize)
where usize: AsPrimitive<T>
{   
    if window == 1 {out.mapv_inplace(|_| f64::NAN); return;} // 如果滚动窗口是1则返回全nan
    let min_periods = min_periods.max(2);
...
pub fn ts_minmaxnorm_1d<T: Number> (arr: ArrayView1<T>, mut out: ArrayViewMut1<f64>, window: usize, min_periods: usize, _out_step: usize)
where usize: AsPrimitive<T>
{   
    if window == 1 {out.mapv_inplace(|_| f64::NAN); return;} // 如果滚动窗口是1则返回全nan
    assert!(window >= min_periods, "滚动window不能小于最小期数");
    ...
    for end in window-1..arr.len() {   
        ...
        unsafe {
            ...
            match (max_idx < start, min_idx < start) {
                ...
                (true, true) => { // 最大和最小值都已经失效，重新找最大和最小值
            ...
            // 检查end位置是否是最大或最小值
            if v >= max {(max, max_idx) = (v, end);}
            if v <= min {(min, min_idx) = (v, end);}
