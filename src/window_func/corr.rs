...
pub fn ts_cov_1d<T: Number> (arr1: ArrayView1<T>, arr2:ArrayView1<T>, mut out: ArrayViewMut1<f64>, window: usize, min_periods: usize, _out_step: usize)
...
{   
    ...
    debug_assert!(out.len() == arr1.len(), "输入数组的维度必须等于输出数组的维度");
...
pub fn ts_corr_1d<T: Number> (arr1: ArrayView1<T>, arr2:ArrayView1<T>, mut out: ArrayViewMut1<f64>, window: usize, min_periods: usize, _out_step: usize)
where usize: AsPrimitive<T>
{   
    if window == 1 {return;} // out默认是全0数列, 如果滚动窗口是1则返回全0
    let min_periods = min_periods.max(2);
    ...
    for i in 0..window-1 { // 安全性：i不会超过arr和out的长度
        ...
        if v_a.notnan() & v_b.notnan() {
            valid_window += 1; 
            sum_a += v_a; sum2_a += v_a * v_a;
            sum_b += v_b; sum2_b += v_b * v_b;
            sum_ab += v_a * v_b;
        };
        unsafe {
            *out.uget_mut(i) = if valid_window >= min_periods {
                ...
                let exey = sum_a * sum_b / v_window.powi(2);
                var_a -= mean_a.powi(2); var_b -= mean_b.powi(2);
