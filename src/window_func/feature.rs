...
pub fn ts_sma_1d<T: Number> (arr: ArrayView1<T>, mut out: ArrayViewMut1<f64>, window: usize, min_periods: usize, _out_step: usize)
...
{   
    ...
    for i in 0..window-1 { // 安全性：i不会超过arr和out的长度
        unsafe {
            ...
            if v.notnan() {sum += v; valid_window += 1};
...
pub fn ts_ewm_1d<T: Number> (arr: ArrayView1<T>, mut out: ArrayViewMut1<f64>, window: usize, min_periods: usize, _out_step: usize)
where usize: AsPrimitive<T>, f64: AsPrimitive<T>
{   // 错位相减核心公式：
...
pub fn ts_wma_1d<T: Number> (arr: ArrayView1<T>, mut out: ArrayViewMut1<f64>, window: usize, min_periods: usize, _out_step: usize)
where usize: AsPrimitive<T>
...
pub fn ts_sum_1d<T: Number> (arr: ArrayView1<T>, mut out: ArrayViewMut1<f64>, window: usize, min_periods: usize, _out_step: usize)
where usize: AsPrimitive<T>
{   
...
pub fn ts_std_1d<T: Number> (arr: ArrayView1<T>, mut out: ArrayViewMut1<f64>, window: usize, min_periods: usize, _out_step: usize)
where usize: AsPrimitive<T>
{   
    if window == 1 {return;} // out默认是全0数列, 如果滚动窗口是1则返回全0
    ...
    for i in 0..window-1 { // 安全性：i不会超过arr和out的长度
        ...
        if v.notnan() {valid_window += 1; sum += v; sum2 += v * v};
        unsafe {
            *out.uget_mut(i) = if valid_window >= min_periods {
...
pub fn ts_skew_1d<T: Number> (arr: ArrayView1<T>, mut out: ArrayViewMut1<f64>, window: usize, min_periods: usize, _out_step: usize)
where usize: AsPrimitive<T>
{   
    if window == 1 {return;} // out默认是全0数列, 如果滚动窗口是1则返回全0
    else if window == 2 {out.mapv_inplace(|_| f64::NAN); return;} // 如果滚动窗口是2则返回全nan
    ...
    for i in 0..window-1 { // 安全性：i不会超过arr和out的长度
        ...
        unsafe {
            *out.uget_mut(i) = if valid_window >= min_periods {
                ...
                std -= mean.powi(2);  // var
                if std == 0f64 {0f64}  // 标准差为0， 则偏度为0
                else {
                    std = std.sqrt(); // std
                    let res = sum3 / v_window; // Ex^3
                    mean /= std;  // mean / std
                    let adjust = (valid_window * (valid_window - 1)).f64().sqrt() / (valid_window - 2).f64();
...
pub fn ts_kurt_1d<T: Number> (arr: ArrayView1<T>, mut out: ArrayViewMut1<f64>, window: usize, min_periods: usize, _out_step: usize)
where usize: AsPrimitive<T>
{   
    if window < 4 {out.mapv_inplace(|_| f64::NAN); return;} // 如果滚动窗口小于4则返回全nan
...
pub fn ts_prod_1d<T: Number> (arr: ArrayView1<T>, mut out: ArrayViewMut1<f64>, window: usize, min_periods: usize, _out_step: usize)
where usize: AsPrimitive<T>
{   
    assert!(window >= min_periods, "滚动window不能小于最小期数");
    assert!(window <= arr.len(), "滚动window不能大于数组长度");
...
pub fn ts_prod_mean_1d<T: Number> (arr: ArrayView1<T>, mut out: ArrayViewMut1<f64>, window: usize, min_periods: usize, _out_step: usize)
where usize: AsPrimitive<T>
{   
    assert!(window >= min_periods, "滚动window不能小于最小期数");
    assert!(window <= arr.len(), "滚动window不能大于数组长度");
    ...
    for i in 0..window-1 { // 安全性：i不会超过arr和out的长度
        unsafe {
            ...
            *out.uget_mut(i) = if valid_window >= min_periods {
                if zero_num == 0 {prod.powf(1. / valid_window.f64())}
                else {0_f64}
            } else { f64::NAN };
        }
    }
