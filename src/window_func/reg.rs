    ...
    for i in 0..window - 1 {
        ...
        unsafe {
            *out.uget_mut(i) = if valid_window >= min_periods {
                ...
                let divisor =
                    (valid_window * (nn_mul_n * (2 * valid_window + 1))).f64() / 6. - sum_t.powi(2);
...
pub fn ts_tsf<T: Number>(
    ...
    usize: AsPrimitive<T>, // 固定window版本，速度较快，但是有nan的话会不准
{
    if window == 1 {
        out.mapv_inplace(|_| f64::NAN);
        return;
