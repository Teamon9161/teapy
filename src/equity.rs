...
use crate::arr::{Arr1, Expr, Number, WrapNdarray};
...
#[pyo3(signature=(pos, opening_cost, closing_cost, init_cash=1_000_000, multiplier=1, leverage=1., slippage=0., ticksize=0., c_rate=3e-4, blowup=false, commision_type=CommisionType::Percent))]
pub unsafe fn calc_ret_single(
        ...
        Zip::from(&pos_arr.0)
            ...
            .map_collect(|&pos, &opening_cost, &closing_cost| {
                ...
                if pos != last_pos {
                    ...
                    // 计算新的理论持仓手数
                    let lot_num =
                        ((cash * leverage * pos.abs()) / (multiplier.f64() * opening_cost)).floor();
                    // 扣除手续费变动
    ...
    Ok(out_expr.to_py(obj).add_obj(obj1).add_obj(obj2))
}
