use numpy::ndarray::{
    Array, ArrayView1, ArrayViewMut1,
};
...
pub fn argsort_1d<T: Number> (arr: ArrayView1<T>, mut out: ArrayViewMut1<usize>) {
    ...
    if c_flag {
        ...
        for (v, v_out) in zip(out_slice, out) {*v_out = *v}
    }
}
...
pub fn rank_1d<T: Number> (arr: ArrayView1<T>, mut out: ArrayViewMut1<f64>) {
    let len = arr.len();
    if len == 0 {return;}
    else if len == 1 {unsafe {*out.uget_mut(0) = 1.}; return;}
    ...
    unsafe {
        ...
        if nan_flag {
            for i in idx..len{*out.uget_mut(*idx_sorted.uget(i)) = f64::NAN;}
        } else {
            sum_rank += cur_rank;
            for i in len - repeat_num..len { // safe because repeat_num <= len
    ...
    unsafe {
        for i in 0..len-1 { // safe because i_max = arr.len()-2 and arr.len() >= 2
            ...
            if v1.isnan() { // 下一个值是nan，说明后面的值全是nan
                ...
                cur_rank += 1;
                for j in 0..repeat_num { // safe because i >= repeat_num
                    *out.uget_mut(*idx_sorted.uget(i-j)) = sum_rank.f64() / (repeat_num * notnan_count).f64()
                }
                idx = i + 1;
                nan_flag = true;
                break;
        ...
        if nan_flag {
            ...
            sum_rank += cur_rank;
            for i in len - repeat_num..len { // safe because repeat_num <= len
                *out.uget_mut(*idx_sorted.uget(i)) = sum_rank.f64() / (repeat_num * notnan_count).f64()
            }
        }
    }
}