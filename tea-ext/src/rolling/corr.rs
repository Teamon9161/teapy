...
use ndarray::{Data, DataMut, ShapeBuilder, Dimension, Ix1, DimMax};
use std::cmp::min;
...
impl<T: Send + Sync, S: Data<Elem = T>, D: Dimension> CorrTs for ArrBase<S, D>
{
    ...
    {   
        ...
        let mut n = 0;
        if !stable {
            self.as_dim1().apply_window_with_to(other.as_dim1(), out, window, |va, vb, va_rm, vb_rm| {
            ...
            self.as_dim1().stable_apply_window_with_to(other.as_dim1(), out, window, |va, vb, va_rm, vb_rm| {
                if va.notnan() && vb.notnan() {
                    n += 1;
                    sum_a.kh_sum(va, c1);
                    sum_b.kh_sum(vb, c2);
