...
def test_ts_ewm(arr, window):
    ...
    res2 = Expr(arr).ts_ewm(window, min_periods=min_periods).eview()
...
def test_ts_wma(arr, window):
    ...
    res2 = Expr(arr).ts_wma(window, min_periods=min_periods).eview()
...
def test_ts_sum(arr, window):
    ...
    res2 = Expr(arr).ts_sum(window, min_periods=min_periods).eview()
...
@given(make_arr(30, unique=True), st.integers(2, 5))
...
@given(make_arr(30), st.integers(2, 5))
def test_ts_var(arr, window):
    # test moving variance
    arr = pd.Series(arr, copy=False)
    min_periods = np.random.randint(2, window + 1)
    res1 = tp.ts_var(arr, window, min_periods=min_periods)
    res2 = Expr(arr).ts_var(window, min_periods=min_periods).eview()
    res3 = arr.rolling(window, min_periods=min_periods).apply(lambda s: s.var())
    assert_allclose3(res1, res2, res3)


@given(make_arr(30, unique=True), st.integers(3, 5))
...
def test_ts_kurt(arr, window):
    ...
    res2 = Expr(arr).ts_kurt(window, min_periods=min_periods).eview()
