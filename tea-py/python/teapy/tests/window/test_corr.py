...
@given(make_arr(30), st.integers(3, 5))
def test_ts_cov(arr, window):
    ...
    res2 = Expr(arr1).ts_cov(arr2, window, min_periods=min_periods).eview()
...
def test_ts_corr(arr, window):
    ...
    res2 = Expr(arr1).ts_corr(arr2, window, min_periods=min_periods).eview()
